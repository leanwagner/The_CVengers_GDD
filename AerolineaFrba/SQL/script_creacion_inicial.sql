...
AS
BEGIN
INSERT INTO AERONAVE (AERONAVE_FABRICANTE, AERONAVE_ESPACIO_ENCOMIENDAS, AERONAVE_MATRICULA, AERONAVE_MODELO, AERONAVE_SERVICIO)
...
SELECT DISTINCT Butaca_Nro, Butaca_Piso, Butaca_Tipo, (SELECT PASAJE_CODIGO FROM PASAJE WHERE PASAJE_CLI_ID =(SELECT CLI_ID FROM CLIENTE WHERE CLI_NOMBRE = Cli_Nombre AND CLI_APELLIDO = Cli_Apellido AND CLI_DIR = Cli_Dir AND CLI_DNI = Cli_Dni AND CLI_FECHA_NAC = Cli_Fecha_Nac AND CLI_MAIL = Cli_Mail AND CLI_TELEFONO = Cli_Telefono) AND PASAJE_VIAJE_ID = (SELECT VIAJE_ID FROM VIAJE WHERE VIAJE_AERONAVE = (SELECT AERONAVE_ID FROM AERONAVE WHERE AERONAVE_FABRICANTE = Aeronave_Fabricante AND AERONAVE_ESPACIO_ENCOMIENDAS = Aeronave_KG_Disponibles AND AERONAVE_MATRICULA = Aeronave_Matricula AND AERONAVE_MODELO = Aeronave_Modelo AND AERONAVE_SERVICIO = (SELECT SERVICIO_ID FROM SERVICIO WHERE SERVICIO_NOMBRE = Tipo_Servicio)) AND VIAJE_FECHA_LLEGADA = FechaLLegada AND VIAJE_FECHA_LLEGADA_ESTIMADA = Fecha_LLegada_Estimada AND VIAJE_FECHA_SALIDA = FechaSalida AND VIAJE_RUTA = (SELECT RUTA_ID FROM RUTA WHERE RUTA_CODIGO = Ruta_Codigo AND Ruta_Ciudad_Destino = RUTA_DESTINO AND Ruta_Ciudad_Origen = RUTA_ORIGEN AND RUTA_PRECIO_BASE_POR_KILO = Ruta_Precio_BaseKG AND RUTA_PRECIO_BASE_POR_PASAJE = Ruta_Precio_BasePasaje)) AND PASAJE_PRECIO = Pasaje_Precio AND PASAJE_FECHA_COMPRA = PASAJE_FECHA_COMPRA), (SELECT VIAJE_ID FROM VIAJE WHERE VIAJE_AERONAVE = (SELECT AERONAVE_ID FROM AERONAVE WHERE AERONAVE_FABRICANTE = Aeronave_Fabricante AND AERONAVE_ESPACIO_ENCOMIENDAS = Aeronave_KG_Disponibles AND AERONAVE_MATRICULA = Aeronave_Matricula AND AERONAVE_MODELO = Aeronave_Modelo AND AERONAVE_SERVICIO = Tipo_Servicio) AND VIAJE_FECHA_LLEGADA = FechaLLegada AND VIAJE_FECHA_LLEGADA_ESTIMADA = Fecha_LLegada_Estimada AND VIAJE_FECHA_SALIDA = FechaSalida AND VIAJE_RUTA = (SELECT RUTA_ID FROM RUTA WHERE RUTA_CODIGO = Ruta_Codigo AND (SELECT CIUDAD_ID FROM CIUDAD WHERE CIUDAD_NOMBRE = Ruta_Ciudad_Destino) = RUTA_DESTINO AND (SELECT CIUDAD_ID FROM CIUDAD WHERE CIUDAD_NOMBRE = Ruta_Ciudad_Origen) = RUTA_ORIGEN AND RUTA_PRECIO_BASE_POR_KILO = Ruta_Precio_BaseKG AND RUTA_PRECIO_BASE_POR_PASAJE = Ruta_Precio_BasePasaje)) FROM GD2C2015.gd_esquema.Maestra
WHERE Butaca_Nro <> 0
END
GO
*/
